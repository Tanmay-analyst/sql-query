...
--1.what is total amount spent by each customer
...
select * from(
select*,rank()over (partition by userid order by created_date asc) rnk from sales)a where rnk=1                 --alias is needed

--4.what is the most purchased item on the menu and how many times it was purchased by all customers?
select userid,count(product_id) cnt from sales where product_id=(
select top 1 product_id /*count(product_id) as cnt*/ from sales
group by product_id
